...
func serve(w http.ResponseWriter, r *http.Request, admit admitHandler) {
	...
	codesFactory := serializer.NewCodecFactory(scheme)
	deserializer := codesFactory.UniversalDeserializer()
	obj, gvk, err := deserializer.Decode(body, nil, &v1.AdmissionReview{})
	if err != nil {
...
func main() {
	...
	fmt.Println("Starting admission application")

	http.HandleFunc("/add-labels", serveAddLabels)

	server := &http.Server{
		Addr:      fmt.Sprintf(":%d", 4567),
...
func serveAddLabels(w http.ResponseWriter, r *http.Request) {
